...
def train_knn_with_tuning(df, num, features, min_minutes=450):
    ...
    test_sizes = np.arange(0.15, 0.85, 0.05)
    ...
    best_idx = np.unravel_index(results_f1.argmax(), results_f1.shape)
    best_k = k_values[best_idx[1]]
    best_test_size = test_sizes[best_idx[0]]

    print(f"\nBest k: {best_k}")
    print(f"Best test_size: {best_test_size:.2f}")
    print(f"Best F1 Score: {results_f1[best_idx]:.4f}")

    # Visualize heatmap
    fig, ax = plt.subplots(figsize=(14, 6))
    sns.heatmap(results_f1, annot=True, fmt='.3f', cmap='YlOrRd', cbar=True,
                xticklabels=k_values, yticklabels=[f'{ts:.2f}' for ts in test_sizes],
                ax=ax, cbar_kws={'label': 'F1 Score'})

    # Highlight best
    rect = plt.Rectangle((best_idx[1], best_idx[0]), 1, 1, fill=False, edgecolor='blue', linewidth=3)
    ax.add_patch(rect)

    plt.title(f'KNN Hyperparameter Tuning - F1 Score\n(Best: k={best_k}, test_size={best_test_size:.2f})',
...
def visualize_knn_with_pca(df, X, knn, scaler, num, features):
    ...
    # LEFT: KNN Group Assignment
    ax1 = axes[0]
    mask_not_top = predictions == 0
    mask_top = predictions == 1

    ax1.scatter(X_pca[mask_not_top, 0], X_pca[mask_not_top, 1],
               c='#2E86AB', alpha=0.6, s=100, label='Not Top Attacker', edgecolors='black', linewidth=0.5)
    ax1.scatter(X_pca[mask_top, 0], X_pca[mask_top, 1],
               c='#FF6B6B', alpha=0.8, s=200, marker='*', label='Top Attacker', edgecolors='darkred', linewidth=2)
    ...
    ax1.set_title('KNN Classification - Colored by Group', fontsize=13, fontweight='bold')
    ax1.legend(fontsize=11, loc='best')
    ax1.grid(alpha=0.3)

    # RIGHT: AttackScore Gradient
    ax2 = axes[1]
    ...
    ax2.scatter(X_pca[mask_top, 0], X_pca[mask_top, 1],
               c='darkred', alpha=0.9, s=250, marker='*', edgecolors='darkred', linewidth=2, label='Top Attacker', zorder=10)

    ax2.set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)', fontsize=12, fontweight='bold')
    ax2.set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)', fontsize=12, fontweight='bold')
    ax2.set_title('Players - Colored by AttackScore', fontsize=13, fontweight='bold')
    plt.colorbar(scatter, ax=ax2, label='AttackScore')
    ...
    # Scree plot
    ax = axes[0]
    ax.bar(range(1, len(pca_full.explained_variance_ratio_) + 1), pca_full.explained_variance_ratio_, color='#2E86AB')
    ...
    # Cumulative
    ax = axes[1]
    ax.plot(range(1, len(cumsum) + 1), cumsum, 'ro-', linewidth=2, markersize=8, label='Cumulative')
    ...
    print(f"2 components explain: {cumsum[1]*100:.1f}% of variance")
